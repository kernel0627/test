...
工程寿命定义为 `T_eng = min(T_func, T_rec, T_burden, T_cap)`。同一天多类终止同时出现时，优先级为 `hmax_depleted > unrecoverable_below_37 > excessive_maintenance_burden > functional_failure > service_cap_reached`。因此，表05同时保留自然功能失效日期和工程最终寿命日期，二者不能混同。

预测起点为 {prediction_start}。主模型 `recovery_ratio_main` 的 neutral 情景中，{ended_main} 台设备在 30 年预测期内触发寿命终止；未触发设备为 {not_reached_main}。

{_md_table(["设备", "Hmax情景", "当前真实rolling365", "功能失效日", "最终寿命日", "最终失效类型", "剩余寿命年", "总寿命年", "状态"], lifetime_rows)}
...
新增 `表08_寿命过长诊断表.csv`，用于解释 30 年主预测中未触发寿命终止的设备为什么寿命较长；在本次工程约束加入后，所有设备都已在 30 年内触发终止，因此该表主要用于解释维护负担、Hmax 变化和 rolling365 走势，并给第三问维护策略优化提供衔接线索。该表采用 `recovery_ratio_main + neutral` 口径，不改变表05主结果。
...
{_md_table(["设备", "30年维护次数", "年均维护次数", "平均间隔", "rolling365最小值", "30年末rolling365", "维护负担", "诊断原因"], diagnostics_rows)}
...
这一版 Q2 已经修正了五个关键问题：回测是真比较，Hmax 是限幅后三情景，大维护触发不再把 K=0 误转成 1，模型推荐逻辑和 fallback 行为一致，并且寿命终止从单一 rolling365 判据升级为功能失效、恢复能力失效、维护负担失效和工程服役上限共同约束。

当前第二问的结论是“按现有维护触发规则继续运行时的工程寿命预测”，不是维护策略优化。A4/A8 是否应加入大维护、维护频率是否要改变、如何降低 excessive maintenance burden，应放到第三问优化模型中处理。
"""
